...
def decode(data, calculate_crc, time_interval, alpha):
    ...
    for i in range(4):
        ...
        MY_str = f"{voltage:7.5f}, {current:7.5f}, {dp:5.3f}, {dn:5.3f}, {temp_ema:6.3f},{energy:.6f}, {capacity:.6f}"
        # print(
        #     f"{t:.3f} {i} {voltage:7.5f} {current:7.5f} {dp:5.3f} {dn:5.3f} "
        #     f"{temp_ema:6.3f} {energy:.6f} {capacity:.6f}"
        # )
        return_arr[i] = np.array([voltage, current, dp, dn, temp_ema, energy, capacity])
        return_str += MY_str + "\n"
    # unknown62 = data[62]  # data[-2]
    # print(f"unknown62 {unknown:02x}")
    # print()
    return return_str, return_arr
...
def collect_5_second_csv(file_name="records.csv"):
    ...
    csv_head = "voltage_V,current_A,dp_V,dn_V,temp_C_ema,energy_Ws,capacity_As"
    f = open(file_name, "w")
    f.write(csv_head + "\n")
    ...
    while iteration < stop/4:
        ...
        try:
            ...
            returnStr = decode(data, crc_calculator, time_interval, alpha)
            # print(returnStr)
            f.write(returnStr)
...
def collect_5_second(file_name="records.npy"):
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("--crc", type=str2bool, help="Enable CRC checks", default=False)
    parser.add_argument("--verbose", type=str2bool, help="Show some extra initalization information", default=False)
    parser.add_argument("--alpha", type=float, help="Set temperature smoothing factor", default=0.9)
    ...
    while iteration < stop/4:
        ...
        try:
            ...
            _, returnArr = decode(data, crc_calculator, time_interval, alpha)
            # for sample in  returnStr.split("\n"):
                # data_array = np.append(data_array, sample.split(","))
            data_array = np.append(data_array, returnArr, axis=0)
            # print(returnStr)
            # f.write(returnStr)
